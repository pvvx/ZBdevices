...
const zclAttrInfo_t pollCtrl_attrTbl[] =
{
	...
	{ ZCL_ATTRID_LONG_POLL_INTERVAL, 	ZCL_DATA_TYPE_UINT32, ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE, (u8*)&g_zcl_pollCtrlAttrs.longPollInterval },
	{ ZCL_ATTRID_SHORT_POLL_INTERVAL, 	ZCL_DATA_TYPE_UINT16, ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE, (u8*)&g_zcl_pollCtrlAttrs.shortPollInterval },
...
//#define FLASH_DEV_NAME_ADDR  (CFG_NV_START_FOR_BLE - 0x1000) // 0x74000 - 0x1000 = 0x73000
...
nv_sts_t zcl_thermostatConfig_save(void)
{
	...
	if(memcmp(&zcl_nv_thermostatUiCfg, &g_zcl_thermostatUICfgAttrs, sizeof(g_zcl_thermostatUICfgAttrs))) {
		...
		zb_setPollRate(g_zcl_pollCtrlAttrs.longPollIntervalMin * POLL_RATE_QUARTERSECONDS);
